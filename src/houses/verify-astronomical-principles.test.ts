...
console.log(`  ASC=${result.angles.ascendant.toFixed(2)}° ↔ DSC=${result.angles.descendant.toFixed(2)}°`);
console.log(`  MC=${result.angles.midheaven.toFixed(2)}° ↔ IC=${result.angles.imumCoeli.toFixed(2)}°`);
assert.ok(Math.abs((result.angles.ascendant + 180) % 360 - result.angles.descendant) < 0.001, 'ASC + 180 = DSC');
assert.ok(Math.abs((result.angles.midheaven + 180) % 360 - result.angles.imumCoeli) < 0.001, 'MC + 180 = IC');
...
for (let i = 0; i < 12; i++) {
  ...
  console.log(`    House ${i + 1}: ${size.toFixed(2)}° (deviation from 30°: ${deviation.toFixed(2)}°)`);
}
...
for (let i = 0; i < 12; i++) {
  ...
  console.log(`  House ${i + 1}: ${wholeSignResult.cusps.cusps[i].toFixed(2)}° (mod 30 = ${cuspMod30.toFixed(2)}°)`);
